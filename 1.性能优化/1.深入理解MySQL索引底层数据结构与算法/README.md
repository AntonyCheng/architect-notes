...
3、主键索引时，叶子节点包含了完整的数据记录。

接下来带着上述特点观察下面两张图，第一张是主键索引示例图，第二张是次要键索引示例图：

![image-20241105090732386](./assets/image-20241105090732386.png)

![image-20241105090735692](./assets/image-20241105090735692.png)

根据示例图可以明显看出平时工作中使用到的`xxxById`操作在执行查询操作时可以一次性把一整条数据查出，因此不会发生回表操作，效率极高。如果我们建立了次要键，通过`xxxByName`操作去查询，首先会通过B+Tree索引树找到该条数据的主键ID，然后再回表操作，通过主键B+Tree索引树找到该条数据的完整信息。
...
**2、为什么非主键索引结构（比如次要键索引结构）叶子节点存储的是主键值？**
...
![image-20241105095632604](./assets/image-20241105095632604.png)
